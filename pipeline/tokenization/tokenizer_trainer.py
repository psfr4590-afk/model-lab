...
class SimpleFallbackTokenizer:
    """Deterministic byte tokenizer used only when HF tokenizers is unavailable."""
    def __init__(self, vocab=None):
        self._vocab = vocab or {"<|pad|>": 0, "<|unk|>": 1, "<|bos|>": 2, "<|eos|>": 3, "<|sep|>": 4, "<|mask|>": 5}
        for i in range(256):
            self._vocab.setdefault(f"<byte:{i}>", 6 + i)
        self._reverse = {v: k for k, v in self._vocab.items()}

    def encode(self, text: str):
        return _Encoded([self._vocab[f"<byte:{b}>"] for b in text.encode("utf-8", errors="replace")])
    ...
    def save(self, path: str):
        Path(path).write_text(json.dumps({"type": "simple-byte", "vocab": self._vocab}, indent=2), encoding="utf-8")
...
class BPETokenizerTrainer:
    ...
    def train(self, corpus_path: str | Path) -> None:
        if not HF_TOKENIZERS_AVAILABLE:
            ...
            tokenizer = SimpleFallbackTokenizer()
            tokenizer.save(str(self._output_path / "tokenizer.json"))
            (self._output_path / "tokenizer_config.json").write_text(json.dumps({"model_type": "simple-byte", "vocab_size": tokenizer.get_vocab_size()}, indent=2), encoding="utf-8")
            log.warning("HF tokenizers unavailable; using deterministic byte-tokenizer fallback")
            return tokenizer
    ...
    def load(self):
        ...
        if not HF_TOKENIZERS_AVAILABLE:
            ...
            if data.get("type") != "simple-byte":
                raise RuntimeError("Tokenizer artifact requires the tokenizers package")
            tok = SimpleFallbackTokenizer(data.get("vocab"))
        else:
